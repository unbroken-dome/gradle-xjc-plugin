...
class CompleteIntegrationTest {
    ...
    fun test(runner: GradleRunner, @GradleProjectDir projectDir: File) {
        ...
        assertThat(buildResult).all {
            task(":library:xjcGenerate").isSuccess()
            task(":library:build").isSuccess()
            task(":consumer:xjcGenerate").isSuccess()
            task(":consumer:build").isSuccess()
        }

        // Should generate books schema classes and episode file in library
        ...
        val classInfo = URLClassLoader(projectDir.resolve("library/build/libs/library.jar"))
